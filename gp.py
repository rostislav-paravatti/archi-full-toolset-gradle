        ...
        with open(file, 'r') as f:
            for line in f:
                if line.startswith('Class-Path:'):
                    class_path = line.strip().split(': ', 1)[1]
                    for jar in class_path.split():
                        if jar.endswith('.jar'):
                            jar_name = os.path.basename(jar)
                            artifact, version = jar_name.rsplit('-', 1)
                            version = version.replace('.jar', '')
                            dependencies.append(('unknown.group', artifact, version))
    except Exception as e:
        print(f"Ошибка при обработке {file}: {e}")
...
def main():
    ...
    manifest_files = find_files(project_dir, ['MANIFEST.MF'])
    ...
    for pom_file in pom_files:
        ...
        # Проверка наличия MANIFEST.MF в той же директории, что и pom.xml
        manifest_file = os.path.join(os.path.dirname(pom_file), 'META-INF', 'MANIFEST.MF')
        if os.path.exists(manifest_file):
            manifest_dependencies = parse_manifest(manifest_file)

        all_dependencies = dependencies + manifest_dependencies
        create_build_gradle(module_path, group_id, version, all_dependencies)
